<template>
    <el-container>
        ...
        <el-header class="header">
            <el-form :inline="true" :model="searchForm" @submit.prevent="search()">
                <el-form-item>
                    <el-input v-model="searchForm.userName" placeholder="用户名" />
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" :icon="Search" @click.prevent="search()">查询</el-button>
                    <el-button type="success" :icon="Plus" @click="showAddOrEdit()">添加</el-button>
                </el-form-item>
            </el-form>
        </el-header>
        <el-main>
                ...
                <el-table-column fixed="right" label="操作" width="120">
                    <template #default="scope">
                        <el-button link type="primary" size="small"
                            @click="showAddOrEdit(scope.row.id, scope.row)">编辑</el-button>
        ...
        <el-footer>
            <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[1, 5, 10, 20, 30]"
    ...
    <el-dialog v-model="show" :title="title">
        <el-form :model="form">
            <el-form-item label="用户名" label-width="140px">
                <el-input v-model="form.username" placeholder="用户名" :disabled="title != '添加用户'" />
            </el-form-item>
            <el-form-item label="密码" label-width="140px" v-if="title == '添加用户'">
...
<script setup>
import { ArrowRight, Search, Plus, Check, Close } from '@element-plus/icons-vue'
...
async function search() {
    ...
    let result = await instance.get(url, { headers: { Authorization: userStore.user.token } });
    console.log(result)
    if (result.data.meta.status != "200") return ElMessage.error(res.data.meta.msg);
...
function save() {
    if (title.value == "添加用户") {
        ...
        instance.post(url, data, { headers: { Authorization: userStore.user.token } }).then(res => {
            console.log(res)
        ...
        instance.put(url, data, { headers: { Authorization: userStore.user.token } }).then(res => {
            console.log(res)
...
function remove(id) {
    ...
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            let url = `users/${id}`;
            instance.delete(url, { headers: { Authorization: userStore.user.token } }).then(res => {
                if (res.data.meta.status != "200") return ElMessage.error(res.data.meta.msg);
